...
function OrderSummaryPanel({ orderItems, totalPrice, shippingFee, finalPrice, onSubmit, submitting }: {
  orderItems: CartItem[]
  totalPrice: number
  shippingFee: number
  finalPrice: number
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-gray-200 p-5">
          ...
          orderItems.map((item) => (
            <li key={item.id} className="flex gap-3">
              ...
              <div className="flex flex-col justify-center gap-0.5 min-w-0">
                {item.brand && <p className="text-xs font-semibold uppercase text-gray-400">{item.brand}</p>}
                <p className="truncate text-sm font-medium text-gray-900">{item.name}</p>
                {item.optionText && <p className="text-xs text-gray-400">{item.optionText}</p>}
                <p className="text-sm font-bold text-gray-900">
                  {(item.price * item.quantity).toLocaleString('ko-KR')}원
                  {item.quantity > 1 && <span className="ml-1 text-xs font-normal text-gray-400">×{item.quantity}</span>}
                </p>
              </div>
            </li>
          ))
      ...
      <button
        onClick={onSubmit}
        disabled={submitting}
        className="w-full rounded bg-gray-900 py-3 text-sm font-bold text-white transition-colors hover:bg-gray-700 disabled:opacity-60"
      >
            ...
            <section>
              ...
              <div className="flex flex-col gap-4 rounded-lg border border-gray-200 p-5">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="mt-[22px] h-[42px] shrink-0 rounded border border-gray-900 px-4 text-sm font-medium text-gray-900 transition-colors hover:bg-gray-50">검색</button>
                </div>
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <select value={form.deliveryMemo} onChange={field('deliveryMemo')} className="rounded border border-gray-300 px-3 py-2.5 text-sm text-gray-700 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 bg-white px-4 py-3 lg:hidden">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            disabled={submitting}
            className="shrink-0 rounded bg-gray-900 px-8 py-3 text-sm font-bold text-white transition-colors hover:bg-gray-700 disabled:opacity-60"
          >
            {submitting ? '처리 중...' : '결제하기'}
          </button>
        </div>
