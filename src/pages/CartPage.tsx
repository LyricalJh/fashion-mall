...
function QuantityStepper({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex items-center rounded border border-gray-300">
      ...
      <span className="flex h-8 w-9 items-center justify-center border-x border-gray-300 text-sm font-medium text-gray-900">
        {value}
      </span>
      <button
  ...
  return (
    <div className="flex gap-3 border-b border-gray-100 py-4 last:border-b-0">
      ...
      <input
        ...
        onChange={onToggle}
        className="mt-1 h-4 w-4 shrink-0 cursor-pointer accent-gray-900"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <div className="flex items-baseline gap-1.5">
          {item.discountRate && (
            <span className="text-xs font-bold text-rose-600">{item.discountRate}%</span>
          )}
        ...
        <div className="mt-1 flex items-center justify-between">
          {item.deliveryText ? (
            <p className="text-xs text-gray-500">{item.deliveryText}</p>
          ) : (
            <span />
          )}
  ...
  return (
    <div className="flex items-center gap-4 border-b border-gray-200 py-3">
      ...
      <button
        ...
        className="text-sm text-gray-500 transition-colors hover:text-gray-900 disabled:opacity-30"
      >
        선택삭제
      </button>
      <span className="text-gray-200">|</span>
      <button disabled className="cursor-not-allowed text-sm text-gray-300">
        품절/종료상품 삭제
      </button>
    </div>
  )
...
function OrderSummary({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-gray-200 p-5">
      ...
      <div className="flex flex-col gap-3 text-sm">
        ...
        {shippingFee > 0 && (
          <p className="text-xs text-gray-400">
            {formatKRW(FREE_SHIPPING_THRESHOLD - totalPrice)} 더 담으면 무료배송
          </p>
        )}
      </div>
      ...
      <Link
        to="/checkout"
        className="block rounded bg-gray-900 py-3 text-center text-sm font-bold text-white transition-colors hover:bg-gray-700"
      >
        구매하기
      </Link>
    </div>
